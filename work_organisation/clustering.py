...
def cluster_whiteboard_cards(session_id: int | str, user=None) -> dict:
    ...
    if not cards.exists():
        return {"status": "empty", "clusters_created": 0, "message": "No notes found in this session to cluster."}

    if cards.count() < 2:
        return {"status": "warning", "clusters_created": 0, "message": "At least 2 notes are needed to synthesize thematic clusters."}
    ...
    try:
        ...
        logger.error(f"Error during whiteboard clustering: {e}", exc_info=True)
        return {
            "status": "error",
            "clusters_created": 0,
            "message": f"LLM inference error: {str(e)}. You can create clusters manually with '➕ New Cluster'."
        }
    ...
    if not clusters_data:
        return {
            "status": "warning",
            "clusters_created": 0,
            "message": "No clusters could be formed from the provided notes. You can add more descriptive notes or create clusters manually."
        }

    # Lookup helpers for matching database IDs or 1-based relative indices
    card_id_map = {c.id: c for c in cards}
    card_idx_map = {idx + 1: c.id for idx, c in enumerate(cards)}
    ...
    for i, cl in enumerate(clusters_data):
        ...
        for card_idx, raw_cid in enumerate(c_card_ids):
            target_cid = None
            try:
                num_cid = int(raw_cid)
                if num_cid in card_id_map:
                    target_cid = num_cid
                elif num_cid in card_idx_map:
                    target_cid = card_idx_map[num_cid]
            except (ValueError, TypeError):
                pass

            if target_cid:
                WhiteboardCard.objects.filter(id=target_cid, session=session).update(
                    cluster=cluster,
                    pos_x=pos_x + 15.0,
    ...
    if not created_clusters:
        return {
            "status": "warning",
            "clusters_created": 0,
            "message": "No clusters were generated by the AI service. You can create clusters manually."
        }
